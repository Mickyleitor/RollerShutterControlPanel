...
bool rtc_on_sync() { return (rtc_time_initialized && !rtc_drift_exceeded()); }
...
void rtc_handler(void) {
    if (rtc_drift_exceeded()) {
        ...
        struct rtcTime_t rtcTime = persistentVars_get_rtcTime();
        rtcTime.myTime           = time(NULL);
        persistentVars_store_rtcTime(&rtcTime);
...
void rtc_set(time_t newTime, long timezoneShift) {
    ...
    struct rtcTime_t rtcTime = { .myTime = newTime, .timezoneShift = timezoneShift };
    persistentVars_store_rtcTime(&rtcTime);
}

/// On boot/soft reset; if valid data in no-init RAM, restore system clock.
void rtc_init() {
    struct timeval tv = { .tv_sec = persistentVars_get_rtcTime().myTime, .tv_usec = 0 };
    settimeofday(&tv, NULL);
