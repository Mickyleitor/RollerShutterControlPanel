...
void SystemFunctionManager()
{
    ...
    if ((nowSecondsUTC % (60 * 60 * 24)) >= (MyWeather.sunriseSecondsUTC + MyWeather.timezoneshift)) {
        ...
        if (settings.ScheduledData[timenow->tm_mon][timenow->tm_mday - 1] == SCHEDULED_DATA_FLAG_PENDING) {
            // Switch to true the notification flag
            settings.ScheduledData[timenow->tm_mon][timenow->tm_mday - 1] |= SCHEDULED_DATA_FLAG_DONE;
